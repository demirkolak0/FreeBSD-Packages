...
if [ $de == "XFCE" ]; then
	pkg install -y xfce lightdm lightdm-gtk-greeter firefox
	echo "proc                    /proc           procfs  rw              0       0" >> /etc/fstab
	sysrc dbus_enable="YES"
	sysrc lightdm_enable="YES"
elif [ $de == "KDE" ]; then
	pkg install -y kde sddm firefox
	sysrc dbus_enable="YES"
	...
	pkg install -y mate lightdm lightdm-gtk-greeter firefox
	echo "proc                    /proc           procfs  rw              0       0" >> /etc/fstab
        sysrc dbus_enable="YES"
	sysrc lightdm_enable="YES"
elif [ $de == "Cinnamon" ]; then
	pkg install -y cinnamon lightdm lightdm-gtk-greeter firefox
	echo "proc                    /proc           procfs  rw              0       0" >> /etc/fstab
        sysrc dbus_enable="YES"
        sysrc lightdm_enable="YES"
elif [ $de == "LXQT" ]; then
	pkg install -y lxqt sddm firefox
	echo "proc                    /proc           procfs  rw              0       0" >> /etc/fstab
        sysrc dbus_enable="YES"
	sysrc sddm_enable="YES"
	read -p "Your default keyboard language will be english. If you want to change it please enter the language. (For exp. for english please enter en_US.) :" $kboardlang
